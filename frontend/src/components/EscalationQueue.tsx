...
export default function EscalationQueue({ escalations, onResolved }: Props) {
  ...
  const resolve = async (id: string, resolution: "approved" | "rejected" | "deferred") => {
    setResolving(id);
    try {
  ...
  return (
    <div className="space-y-3">
      {escalations.map(esc => (
        <div key={esc.id} className="bg-slate-800/60 border border-orange-500/20 rounded-xl p-4 fade-in">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <SeverityBadge severity={esc.event.severity} />
                <span className="text-xs font-mono text-slate-400">{esc.event.service} · {esc.event.resource}</span>
              </div>
              <p className="text-sm text-slate-200">{esc.event.message}</p>
              <p className="text-xs text-slate-400 mt-1">{esc.analysis?.root_cause}</p>
              <p className="text-xs text-slate-500 mt-0.5 font-mono">ID: {esc.id}</p>
            </div>
            <div className="flex gap-2 flex-shrink-0">
              {resolving === esc.id ? (
                ...
                <>
                  <button
                    onClick={() => resolve(esc.id, "approved")}
                  ...
                  <button
                    onClick={() => resolve(esc.id, "rejected")}
                  ...
                  <button
                    onClick={() => resolve(esc.id, "deferred")}
